...
module.exports = {
    commandInfo: {
        name: "additemtoall",
        description: "gives a selected item or vbucks to every account",
        options: [
            {
                name: "cosmeticname",
                description: "the name of the cosmetic you want to give to all users",
                required: false,
                type: 3
            },
            {
                name: "vbucks",
                description: "the amount of vbucks you want to give to all users (can be negative to deduct vbucks)",
                required: false,
                type: 4
            },
            {
        ...
        try {
            ...
            for (const user of allUsers) {
                try {
                    ...
                    log.error(`Error processing user ${user.accountId}: ${err.message}`);
                    failCount++;
                }
            }
            ...
            const embed = new MessageEmbed()
                .setTitle(cosmeticname ? "Cosmetic Gift Sent to All" : "V-Bucks Added to All")
                .setDescription(
                    cosmeticname 
                        ? `Successfully gave the cosmetic **${cosmeticname}** to **${successCount}** account(s)`
                        : `Successfully added **${vbucks}** V-Bucks to **${successCount}** account(s)`
                )
                .setColor("GREEN")
                .addFields(
