...
app.use("/fortnite/api/game/v2/profile/*/client", (req, res, next) => {
    if (!req.query.profileId && req.params && req.params[0]) {
        req.query.profileId = req.params[0];
    }
                    ...
                    ApplyProfileChanges.push(
                        {
                            ...
                            "itemId": key,
                            "quantity": profile.items[key].quantity          // just leave this commented out should fix the negetive vbucks bug 
                        },
//                        {
 //                           "changeType": "itemQuantityChanged",
  //                          "itemId": key,
...
app.post("/fortnite/api/game/v2/profile/*/client/PurchaseCatalogEntry", verifyToken, async (req, res) => {
    ...
    let athena = profiles.profiles["athena"];
    let profile0 = profiles.profiles["profile0"];
    let currencyProfile = profiles.profiles["common_core"] || profiles.profiles["profile0"];
    log.debug(`PurchaseCatalogEntry: Validated profile for profileId: ${req.query.profileId}`);

    if (!["common_core", "profile0", "athena"].includes(req.query.profileId)) {
    ...
    const purchaseQuantity = Number(req.body.purchaseQuantity ?? 1);
    ...
    req.body.purchaseQuantity = purchaseQuantity;

    if (!profile.items) profile.items = {};
    if (!currencyProfile.items) currencyProfile.items = {};
    if (!athena.items) athena.items = {};
    if (!profile0.items) profile0.items = {};

    if (!currencyProfile.stats) currencyProfile.stats = { attributes: {} };
    if (!currencyProfile.stats.attributes) currencyProfile.stats.attributes = {};
    if (!profile.stats) profile.stats = { attributes: {} };
    if (!profile.stats.attributes) profile.stats.attributes = {};
    if (!profile0.stats) profile0.stats = { attributes: {} };
    if (!profile0.stats.attributes) profile0.stats.attributes = {};
    ...
    const priceEntry = findOfferId.offerId?.prices?.[0];
    log.debug(`PurchaseCatalogEntry: Offer lookup result name=${findOfferId.name} itemGrants=${(findOfferId.offerId?.itemGrants || []).length} prices=${JSON.stringify(findOfferId.offerId?.prices || [])}`);
    if (!priceEntry) {
        log.debug(`PurchaseCatalogEntry: Offer ${req.body.offerId} has no price data`);
        return error.createError(
            "errors.com.epicgames.fortnite.id_invalid",
            `Offer ID (id: '${req.body.offerId}') has no price data`,
            [req.body.offerId], 16027, undefined, 400, res
        );
    }

    const currencyType = String(priceEntry.currencyType || "mtx").toLowerCase();
    const basePrice = Number(priceEntry.finalPrice || 0);
    const totalPrice = basePrice * purchaseQuantity;
            ...
            if (req.body.offerId == BattlePass.battlePassOfferId || req.body.offerId == BattlePass.battleBundleOfferId || req.body.offerId == BattlePass.tierOfferId) {
                ...
                if (req.body.offerId == BattlePass.battlePassOfferId || req.body.offerId == BattlePass.battleBundleOfferId || req.body.offerId == BattlePass.tierOfferId) {
                    if (currencyType == "mtxcurrency") {
                        let paid = false;
                        for (let key in currencyProfile.items) {
                            const templateId = String(currencyProfile.items[key]?.templateId || "").toLowerCase();
                            if (!templateId.startsWith("currency:mtx")) continue;
                            let currencyPlatform = String(currencyProfile.items[key]?.attributes?.platform || "").toLowerCase();
                            let currentMtxPlatform = String(currencyProfile.stats.attributes.current_mtx_platform || "shared").toLowerCase();
                            if ((currencyPlatform !== currentMtxPlatform) && (currencyPlatform !== "shared")) continue;
                            if (currencyProfile.items[key].quantity < totalPrice) {
                                log.debug(`PurchaseCatalogEntry: Insufficient currency: required ${totalPrice}, available ${currencyProfile.items[key].quantity}`);
                                return error.createError("errors.com.epicgames.currency.mtx.insufficient", `You cannot afford this item (${totalPrice}), you only have ${currencyProfile.items[key].quantity}.`, [`${totalPrice}`, `${currencyProfile.items[key].quantity}`], 1040, undefined, 400, res);
                            }
                
                            currencyProfile.items[key].quantity -= totalPrice;
                            ...
                            ApplyProfileChanges.push({
                                "changeType": "itemQuantityChanged",
                                "itemId": key,
                                "quantity": currencyProfile.items[key].quantity
                            });
                            if (profile0.items[key]) {
                                ApplyProfileChanges.push({
                                    "changeType": "itemQuantityChanged",
                            ...
                            paid = true;
                            log.debug(`PurchaseCatalogEntry: Currency deducted: ${totalPrice}, remaining ${currencyProfile.items[key].quantity}`);
                            break;
                        }
    ...
    switch (true) {
        case /^BR(?:Daily|Weekly|Season|Featured|Standalone)Storefront(?:_|$)/.test(findOfferId.name):
            Notifications.push({
            ...
            if (currencyType == "mtxcurrency") {
                ...
                for (let key in currencyProfile.items) {
                    const itemTemplateId = String(currencyProfile.items[key]?.templateId || "").toLowerCase();
                    if (!itemTemplateId.startsWith("currency:mtx")) continue;

                    const currencyPlatform = String(currencyProfile.items[key]?.attributes?.platform || "").toLowerCase();
                    const currentPlatform = String(currencyProfile.stats.attributes.current_mtx_platform || "shared").toLowerCase();

                    if ((currencyPlatform !== currentPlatform) && (currencyPlatform !== "shared")) continue;

                    if (currencyProfile.items[key].quantity < basePrice) {
                        log.debug(`PurchaseCatalogEntry: Insufficient currency: required ${basePrice}, available ${currencyProfile.items[key].quantity}`);
                        return error.createError(
                            "errors.com.epicgames.currency.mtx.insufficient",
                            `You cannot afford this item (${basePrice}), you only have ${currencyProfile.items[key].quantity}.`,
                            [`${basePrice}`, `${currencyProfile.items[key].quantity}`], 1040, undefined, 400, res
                        );
                    }

                    currencyProfile.items[key].quantity -= basePrice;
                    if (profile0.items[key]) {
                        profile0.items[key].quantity -= basePrice;
                    }
                    ...
                    ApplyProfileChanges.push(
                        {
                            "changeType": "itemQuantityChanged",
                            "itemId": key,
                            "quantity": currencyProfile.items[key].quantity         /// leave commanted out plsssss fixes vbucks
                        },
//                        {
 //                           "changeType": "itemQuantityChanged",
  //                          "itemId": key,
  //                          "quantity": profile0.items[key].quantity
     //                   }
                    ...
                    paid = true;
                    log.debug(`PurchaseCatalogEntry: Currency deducted: ${basePrice}, remaining ${currencyProfile.items[key].quantity}`);
                    break;
                }

                if (!paid && basePrice > 0) {
                    log.debug(`PurchaseCatalogEntry: Insufficient currency: required ${basePrice}, no currency available`);
                    return error.createError(
                        "errors.com.epicgames.currency.mtx.insufficient",
                        `You cannot afford this item (${basePrice}).`,
                        [`${basePrice}`], 1040, undefined, 400, res
                    );
                }
                ...
                if (findOfferId.offerId.itemGrants.length != 0) {
                    ...
                    currencyProfile.stats.attributes.mtx_purchase_history.purchases.push({"purchaseId":purchaseId,"offerId":`v2:/${purchaseId}`,"purchaseDate":new Date().toISOString(),"freeRefundEligible":false,"fulfillments":[],"lootResult":Notifications[0].lootResult.items,"totalMtxPaid":basePrice,"metadata":{},"gameContext":""})
                    if (!profile0.stats) profile0.stats = { attributes: {} };
                    if (!profile0.stats.attributes) profile0.stats.attributes = {};
                    if (!profile0.stats.attributes.mtx_purchase_history) profile0.stats.attributes.mtx_purchase_history = { purchases: [] };
                    profile0.stats.attributes.mtx_purchase_history.purchases.push({"purchaseId":purchaseId,"offerId":`v2:/${purchaseId}`,"purchaseDate":new Date().toISOString(),"freeRefundEligible":false,"fulfillments":[],"lootResult":Notifications[0].lootResult.items,"totalMtxPaid":basePrice,"metadata":{},"gameContext":""})

                    ApplyProfileChanges.push(
                        {
                            "changeType": "statModified",
                            "name": "mtx_purchase_history",
                            "value": currencyProfile.stats.attributes.mtx_purchase_history
                        },
                        {
                            "changeType": "statModified",
                            "name": "mtx_purchase_history",
            ...
            functions.recordTodayPopularPurchase(findOfferId.offerId.itemGrants || []);
            log.debug(`PurchaseCatalogEntry: Successfully processed storefront purchase`);

            break;
        default:
            log.debug(`PurchaseCatalogEntry: Storefront ${findOfferId.name} is not handled by PurchaseCatalogEntry`);
            if (/^BR/i.test(findOfferId.name)) {
                return error.createError(
                    "errors.com.epicgames.fortnite.id_invalid",
                    `Storefront ${findOfferId.name} is not supported for purchase routing`,
                    [findOfferId.name], 16027, undefined, 400, res
                );
            }
    }
...
app.post("/fortnite/api/game/v2/profile/*/client/:operation", verifyToken, async (req, res) => {
    ...
    if (req.query.profileId == "athena") {
        const defaultAthenaPath = path.join(__dirname, "../Config/DefaultProfiles/athena.json");
        let defaultAthenaJson = fs.readFileSync(defaultAthenaPath, "utf8");
        if (defaultAthenaJson.charCodeAt(0) == 0xFEFF) defaultAthenaJson = defaultAthenaJson.slice(1);
        const defaultAthena = JSON.parse(defaultAthenaJson);
        const currentItemCount = Object.keys(profile.items || {}).length;
        const defaultItemCount = Object.keys(defaultAthena.items || {}).length;

        if (currentItemCount < defaultItemCount) {
            profile.items = defaultAthena.items;
