...
var pings = [];
var vcParticipants = {};
global.vcParticipants = vcParticipants;
var vcInfo = {};

function getUserInvites(accountId) {
  return Object.values(global.parties || {}).flatMap((party) => {
    if (!party || !Array.isArray(party.invites)) return [];
    return party.invites.filter((invite) => invite.sent_to == accountId);
  });
}

function getUserPendingIntentions(accountId) {
  return Object.values(global.parties || {}).flatMap((party) => {
    if (!party || !Array.isArray(party.intentions)) return [];
    return party.intentions.filter((intention) => intention.requestee_id == accountId || intention.requestee_id === accountId);
  });
}

async function resolveQuery(query) {
  if (query && typeof query.lean === "function") return query.lean();
  if (query && typeof query.cache === "function") return query.cache();
  return query;
}

function normalizePartyAccountId(accountId) {
  return String(accountId || "").split("@prod")[0];
}

function getPartyMemberIndex(party, accountId) {
  if (!party || !Array.isArray(party.members)) return -1;
  return party.members.findIndex((member) => member && member.account_id == accountId);
}

function getPartyMember(party, accountId) {
  const index = getPartyMemberIndex(party, accountId);
  return index === -1 ? null : party.members[index];
}
...
app.get("/party/api/v1/Fortnite/user/:accountId/notifications/undelivered/count", verifyToken, async (req, res) => {
    ...
    const pendingCount = getUserPendingIntentions(req.params.accountId).length;
    const inviteCount = p ? p.invites.filter(p => p.sent_to == req.params.accountId).length : getUserInvites(req.params.accountId).length;
...
app.get("/party/api/v1/Fortnite/user/:accountId", verifyToken, async (req, res) => {
  ...
  res.json({
    ...
    "pending": getUserPendingIntentions(req.params.accountId),
    "invites": getUserInvites(req.params.accountId),
...
app.patch("/party/api/v1/Fortnite/parties/:pid", verifyToken, async (req, res) => {
  ...
  const metaDelete = Array.isArray(meta.delete) ? meta.delete : [];
  const metaUpdate = typeof meta.update === "object" && meta.update ? meta.update : {};

  if (req.body.config) {
    Object.assign(newp.config, req.body.config);
  }
...
app.patch("/party/api/v1/Fortnite/parties/:pid/members/:accountId/meta", verifyToken, async (req, res) => {
  ...
  if (!member) return error.createError("errors.com.epicgames.party.not_found", `Member ${req.params.accountId} does not exist in party ${req.params.pid}!`, undefined, 51002, undefined, 404, res);
  ...
  const deleteProps = Array.isArray(req.body.delete) ? req.body.delete : [];
  const updateProps = typeof req.body.update === "object" && req.body.update ? req.body.update : {};

  for (var prop of deleteProps) {
    delete member.meta[prop];
  }

  for (var prop of Object.keys(updateProps)) {
    member.meta[prop] = updateProps[prop];
  }
...
app.delete("/party/api/v1/Fortnite/parties/:pid/members/:accountId", verifyToken, async (req, res) => {
  ...
  const targetMember = getPartyMember(newp, req.params.accountId);
  if (!targetMember) return error.createError("errors.com.epicgames.party.not_found", `Member ${req.params.accountId} does not exist in party ${req.params.pid}!`, undefined, 51002, undefined, 404, res);

  const actingMember = getPartyMember(newp, req.user.accountId);
  const isSelfRemoval = req.user.accountId == req.params.accountId;
  const isCaptainAction = actingMember && actingMember.role === "CAPTAIN";

  if (!isSelfRemoval && !isCaptainAction) {
    return error.createError("errors.com.epicgames.party.unauthorized", `User ${req.user.accountId} is not allowed to delete member ${req.params.accountId}!`, undefined, 51015, undefined, 403, res);
  }

  if (!isSelfRemoval && targetMember.role === "CAPTAIN") {
    return error.createError("errors.com.epicgames.party.unauthorized", `User ${req.user.accountId} is not allowed to delete captain ${req.params.accountId}!`, undefined, 51015, undefined, 403, res);
  }

  const mIndex = getPartyMemberIndex(newp, req.params.accountId);
  newp.members.forEach(async (member) => {
    functions.sendXmppMessageToId({
        account_id: req.params.accountId,
        member_state_update: {},
...
app.post("/party/api/v1/Fortnite/user/:accountId/pings/:pingerId", verifyToken, async (req, res) => {
  ...
  functions.sendXmppMessageToId({
    ...
    meta: req.body.meta,
    ns: "Fortnite",
    pinger_dn: (await resolveQuery(User.findOne({ accountId: req.params.pingerId })))?.username || req.params.pingerId,
...
app.post("/party/api/v1/Fortnite/user/:accountId/pings/:pingerId/join", verifyToken, async (req, res) => {
  ...
  if (!newp) return error.createError("errors.com.epicgames.party.not_found", `Party for ping sender ${query[0].sent_by} does not exist!`, undefined, 51002, undefined, 404, res);
...
app.post('/party/api/v1/Fortnite/parties/:pid/invites/:accountId', verifyToken, async (req, res) => {
  ...
  var friends = await resolveQuery(Friends.findOne({ accountId: req.user.accountId }));
...
app.post([
  '/party/api/v1/Fortnite/parties/:pid/invites/:accountId/accept',
  '/party/api/v1/Fortnite/parties/:pid/invites/:accountId/*/accept'
], verifyToken, async (req, res) => {
  var newp = global.parties[req.params.pid];
  if (!newp) return error.createError("errors.com.epicgames.party.not_found", `Party ${req.params.pid} does not exist!`, undefined, 51002, undefined, 404, res);

  const invite = newp.invites.find((p) => p.sent_to == req.params.accountId);
  if (!invite) return error.createError("errors.com.epicgames.party.not_found", `Invite for ${req.params.accountId} does not exist!`, undefined, 51002, undefined, 404, res);

  const memberExists = newp.members.some((m) => m.account_id == req.params.accountId);
  if (memberExists) {
    newp.invites = newp.invites.filter((p) => p.sent_to != req.params.accountId);
    res.json({ status: "JOINED", party_id: newp.id });
    return;
  }

  const connection = req.body.connection || { id: `${req.params.accountId}@prod` };
  const member = {
    account_id: normalizePartyAccountId(connection.id || req.params.accountId),
    meta: req.body.meta || {},
    connections: [{
      id: connection.id || `${req.params.accountId}@prod`,
      connected_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
      yield_leadership: Boolean(connection.yield_leadership),
  ...
  const v = newp.meta['Default:RawSquadAssignments_j'] ? 'Default:RawSquadAssignments_j' : 'RawSquadAssignments_j';
  if (newp.meta[v]) {
    var rsa = JSON.parse(newp.meta[v]);
    rsa.RawSquadAssignments.push({ memberId: member.account_id, absoluteMemberIdx: newp.members.length - 1 });
    newp.meta[v] = JSON.stringify(rsa);
    newp.revision++;
  }

  const captain = newp.members.find((partyMember) => partyMember.role === "CAPTAIN") || { account_id: "" };
  res.json({ status: "JOINED", party_id: newp.id });
  newp.members.forEach(async (partyMember) => {
    functions.sendXmppMessageToId({
      account_dn: member.meta['urn:epic:member:dn_s'],
      account_id: member.account_id,
      connection: {
        connected_at: new Date().toISOString(),
        id: member.connections[0].id,
        meta: member.connections[0].meta,
        updated_at: new Date().toISOString(),
      },
    ...
    functions.sendXmppMessageToId({
      ...
      party_state_removed: [],
      party_state_updated: {
        [v]: JSON.stringify(rsa || JSON.parse(newp.meta[v] || '{"RawSquadAssignments":[]}'))
      },
      party_sub_type: newp.meta['urn:epic:cfg:party-type-id_s'],
      party_type: "DEFAULT",
      revision: newp.revision,
...
app.get("/party/api/v1/Fortnite/members/:accountId/intentions", verifyToken, async (req, res) => {
  const intentions = getUserPendingIntentions(req.params.accountId);
  res.json(intentions);
});

app.get("/party/api/v1/Fortnite/user/:accountId/intentions", verifyToken, async (req, res) => {
  const intentions = getUserPendingIntentions(req.params.accountId);
  res.json(intentions);
});

app.get("/party/api/v1/Fortnite/members/:accountId/intentions/:senderId", verifyToken, async (req, res) => {
  const party = Object.values(global.parties).find(x => x.members.findIndex(m => m.account_id == req.params.senderId) != -1);
  ...
  const intention = (party.intentions || []).find((item) => item.requester_id == req.params.senderId && item.requestee_id == req.params.accountId);
  if (!intention) return error.createError("errors.com.epicgames.party.not_found", `Intention ${req.params.senderId} does not exist!`, undefined, 51002, undefined, 404, res);

  res.json(intention);
});

app.get("/party/api/v1/Fortnite/user/:accountId/intentions/:senderId", verifyToken, async (req, res) => {
  const party = Object.values(global.parties).find(x => x.members.findIndex(m => m.account_id == req.params.senderId) != -1);
  if (!party) return error.createError("errors.com.epicgames.party.not_found", `Party does not exist!`, undefined, 51002, undefined, 404, res);
...
app.post("/party/api/v1/Fortnite/members/:accountId/intentions/:senderId", verifyToken, async (req, res) => {
  ...
  var friends = await resolveQuery(Friends.findOne({ accountId: req.params.accountId }));
  ...
  party.intentions = Array.isArray(party.intentions) ? party.intentions : [];
  const existing = party.intentions.find((item) => item.requester_id == req.params.senderId && item.requestee_id == req.params.accountId);
  if (existing) {
    existing.sent_at = intention.sent_at;
    existing.expires_at = intention.expires_at;
    existing.meta = req.body;
    res.json(existing);
    return;
  }
...
app.post("/party/api/v1/Fortnite/user/:accountId/intentions/:senderId", verifyToken, async (req, res) => {
  var party = Object.values(global.parties).find(x => x.members.findIndex(m => m.account_id == req.params.senderId) != -1);
  if (!party) return error.createError("errors.com.epicgames.party.not_found", `Party does not exist!`, undefined, 51002, undefined, 404, res);
  const sender = party.members.find(x => x.account_id == req.params.senderId);
  const captain = party.members.find((member) => member.role === "CAPTAIN");
...
app.post("/party/api/v1/Fortnite/parties/:pid/members/:accountId/conferences/connection", verifyToken, async (req, res) => {
  ...
  const normalizedAccountId = normalizePartyAccountId(accountId);

  let { vivox, rtcp } = req.body.providers || {};
  const party = global.parties[pid];
  if (!party) return error.createError("errors.com.epicgames.party.not_found", `Party ${pid} does not exist!`, undefined, 51002, undefined, 404, res);
  ...
  if (rtcp && !vivox) {
    ...
    const partyMemberIds = Array.isArray(party.members)
      ? party.members
          .map((member) => String(member?.account_id || '').split('@prod')[0])
          .filter(Boolean)
      : [];

    const participantList = [...partyMemberIds, ...vcParticipants[pid].map((member) => String(member?.puid || '').split('@prod')[0])]
      .filter(Boolean)
      .reduce((unique, puid) => {
        if (!unique.some((item) => item.puid === puid)) {
          unique.push({ puid, clientIP: req.ip, hardMuted: false });
        }
        return unique;
      }, []);

    vcParticipants[pid] = participantList;
    global.vcParticipants = vcParticipants;

    var p = {
      puid: normalizedAccountId,
      clientIP: req.ip,
      hardMuted: false
    };
    ...
    rtcp = {
      participant_token: vcInfo[pid].tokens[normalizedAccountId],
      client_base_url: vcInfo[pid].url,
      room_name: vcInfo[pid].name
    };
  } else {
    const channel_uri = `sip:confctl-g-epicgames.p-${pid}@mtu1xp.vivox.com`;
    const user_uri = `sip:.epicgames.${normalizedAccountId}.@mtu1xp.vivox.com`;
