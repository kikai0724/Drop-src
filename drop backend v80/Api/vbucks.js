...
app.get("/api/Shard/vbucks", async (req, res) => {
    ...
    try {
        ...
        common_core.items[purchaseId] = {
            ...
            "attributes": {
                ...
                "lootList": lootList,
                "params": {
                    "userMessage": `Thank you for playing Drop!`
                },
        ...
        try {
            const notificationPayload = functions.buildVbucksNotificationPayload({
                ...
                reason,
                killCount: 1,
                message: `You received ${addValue} V-Bucks.${reason ? ` (${reason})` : ''} (Kill count: 1)`
            });
            functions.sendXmppMessageToId(notificationPayload, user.accountId);
        } catch (notifyErr) {
            log.error(`Failed to send V-Bucks notification: ${notifyErr.message}`);
        }
...
app.get("/api/Shard/hype/kill/:username/:reason/:apiKey", async (req, res) => {
